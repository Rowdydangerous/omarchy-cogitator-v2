...
Item {
  ...
  // Cogitator: the Applications experience lives in the Application
  // Cogitator console. Any navigation into the apps submenu hands off
  // instead of showing the stock app list.
  function redirectApps(id) {
    if (id !== "apps") return false
    root.cancel()
    Quickshell.execDetached(["omarchy-shell", "cogitator-rite", "openLauncher"])
    return true
  }

  function setActiveMenu(id, pushHistory, fromPointer) {
    if (root.redirectApps(id)) return
    panel.freezeCardTop()
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (row.kind === "menu" || row.kind === "link") {
      ...
      // Cogitator: application execution belongs to the Application
      // Cogitator console. Hand off instead of launching inline.
  ...
  function openRoute(initialMenu) {
    ...
    if (root.redirectApps(id)) return "ok"
  ...
  PanelWindow {
    ...
    MouseArea {
      ...
      BorderSurface {
        ...
        // Cogitator frame: chamfered bottom-right corner + corner brackets
        // over the square card. Presentation only; sizing stays stock.
        Shape {
          anchors.fill: parent
        ...
        Row {
          ...
          Rectangle { width: 18; height: 3; color: root.border }
        }
        Rectangle {
          anchors.top: parent.top
          anchors.left: parent.left
      ...
      Column {
        ...
        Rectangle {
          ...
          Text {
            ...
            text: (root.filterText || (root.dmenuActive ? (root.dmenuPrompt + "…") : ((root.item(root.activeMenu) ? (root.item(root.activeMenu).title || root.item(root.activeMenu).label) : "Go") + "…"))).toUpperCase()
            color: root.foreground
            opacity: root.filterText ? 1 : 0.85
            font.family: "Monaspace Xenon Frozen, JetBrainsMono Nerd Font, monospace"
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              // Cogitator cursor tick: phosphor edge mark on the active row.
              Rectangle {
